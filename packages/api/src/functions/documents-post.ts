...
import { HttpRequest, type HttpResponseInit, type InvocationContext, app } from '@azure/functions';
...
export async function postDocuments(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    context.log(`RawDocument: ${JSON.stringify(rawDocument, null, 2)}`); // Convert to string for logging
    ...
    context.log('RawDocument is empty.');
    return ok({ message: ` Error when processing document-post request: RawDocument is empty for ${filename}` });
